...
- 📜 **Paper:** Yu Xue(Supervisor), **Pengcheng Jiang**, Zhenman Zhang, Mengchu Zhou. Neural architecture search based on a surrogate model with similarity measurement[J]. *IEEE Transactions on Systems, Man and Cybernetics: Systems*. **(Submited on Jul. 19, 2025)** ``Trans`` ``SCI`` ``中科院一区TOP`` ``JCR Q1`` ``IF：8.6`` ``CCF-B`` ``CAAI-B``
- 📜 **Paper:** Yu Xue(Supervisor), Chenchen Zhu, **Pengcheng Jiang**. A multi-objective evolutionary algorithm based on bi-population with uniform sampling for neural architecture search[J]. *IEEE Transactions on Neural Networks and Learning Systems*. **(R1 Submited on Aug. 20, 2025)**
- 📜 **论文：**Yu Xue(Supervisor), **Pengcheng Jiang**, Chenchen Zhu, MengChu Zhou, Mohamed Wahib, Moncef Gabbouj. A pairwise comparison relation-assisted multi-objective evolutionary neural architecture search method with multi-population mechanism[J]. *IEEE Transactions on Systems, Man and Cybernetics: Systems*. **(R5 Submited on Aug. 31, 2025)** [[Arxiv]](https://arxiv.org/abs/2407.15600) ``Trans`` ``SCI`` ``中科院一区TOP`` ``JCR Q1`` ``IF：8.6`` ``CCF-B`` ``CAAI-B``  ``Cited: 4``
...
### Published
- 📜 **Paper:** Yixia Zhang, Ferrante Neri, Xilu Wang, **Pengcheng Jiang**. Evolutionary channel pruning for StyleGANs[C]. *28th European Conference on Artificial Intelligence* **(Accepted)**
