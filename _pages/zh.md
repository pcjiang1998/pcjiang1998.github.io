...
- 📜 **论文：**Yu Xue(导师), **Pengcheng Jiang**, Chenchen Zhu, Yong Zhang, Dunwei Gong. A multi-objective evolutionary algorithm based on bi-population with uniform sampling for neural architecture search[J]. *IEEE Transactions on Neural Networks and Learning Systems*. (2025年8月20日提交第1轮修改) ``Trans`` ``SCI`` ``中科院一区TOP`` ``JCR Q1`` ``IF：8.9`` ``CCF-B`` ``CAAI-A``
